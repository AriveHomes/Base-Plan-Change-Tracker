(function () {
  ...
  function renderPlans() {
    ...
    els.planList.innerHTML = list.map(plan => {
      const changes = changesFor(plan.name);
      const open = changes.filter(c => !isCompleteStatus(c.status)).length;
      const badge = !changes.length ? '<span class="badge">No changes</span>' : open ? `<span class="badge gold">${open} open</span>` : '<span class="badge">Complete</span>';
      return `<button class="plan-card ${state.selected === plan.name ? 'active' : ''}" data-plan="${escapeHtml(plan.name)}" type="button">
        <div><div class="plan-title">${escapeHtml(plan.name)}</div><div class="plan-sub">${changes.length} change${changes.length === 1 ? '' : 's'} • Total sales: ${escapeHtml(plan.totalSales || '0')}</div></div>${badge}
      </button>`;
    }).join('');
  ...
  function renderDetails() {
    ...
    if (!plan) {
      els.details.innerHTML = '<div class="empty-main"><img src="assets/arive-mark.png" class="empty-logo large" alt="Arive Homes mark" /><h2>No plan selected</h2><p>Select a floorplan on the left to view its change history.</p></div>';
      return;
    }
    ...
    <div class="info-card"><h3>Change History</h3>${changes.length ? `<div class="change-list">${changes.map(changeCard).join('')}</div>` : '<div class="empty-state"><img src="assets/arive-mark.png" class="empty-logo" alt="Arive Homes mark" /><strong>No changes logged</strong><span>Click Add Change to create the first record for this floorplan.</span></div>'}</div>`;
  ...
  function changeCard(c) {
    const klass = c.status === 'Cancelled' ? 'cancelled' : c.status === 'On Hold' ? 'hold' : '';
    return `<article class="change-card ${klass}">
      <div class="change-top">
        <div><div class="change-title">${escapeHtml(c.description || '(No description)')}</div><div class="meta"><span>${escapeHtml(c.changeDate || 'No date')}</span><span>${escapeHtml(c.changeType || 'Other')}</span><span>${escapeHtml(c.status || 'Proposed')}</span>${c.costImpact ? `<span>${escapeHtml(c.costImpact)}</span>` : ''}</div></div>
  ...
  function openModal(floorplan, change) {
    ...
    $('#status').value = change ? (change.status || 'Proposed') : 'Proposed';
  ...
  function normalizeChange(c) {
    return {
      ...
      status: String(c.status || 'Proposed').trim(),
  ...
  function isCompleteStatus(s) { return /^(complete|approved)$/i.test(String(s || '').trim()); }
