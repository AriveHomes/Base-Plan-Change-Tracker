(function () {
  ...
  function init() {
    els.status = $('#statusBanner');
    els.sheetLink = $('#sheetLink');
    els.refreshBtn = $('#refreshBtn');
    els.planList = $('#planList');
    els.details = $('#detailsPanel');
    els.search = $('#searchBox') || $('#searchInput');
    els.statusFilter = $('#statusFilter');
    els.typeFilter = $('#typeFilter');
    els.statPlans = $('#statPlans');
    els.statChanges = $('#statChanges');
    els.statOpen = $('#statOpen');
    els.statComplete = $('#statComplete');
    els.modal = $('#modalBackdrop');
    els.form = $('#changeForm');
    els.closeModal = $('#closeModal');
    els.cancelBtn = $('#cancelBtn');
    els.archiveBtn = $('#archiveBtn');

    if (els.sheetLink) els.sheetLink.href = SHEET_URL;
    if (els.refreshBtn) els.refreshBtn.addEventListener('click', () => loadData(true));
    if (els.search) els.search.addEventListener('input', renderPlans);
    if (els.statusFilter) els.statusFilter.addEventListener('change', renderPlans);
    if (els.typeFilter) els.typeFilter.addEventListener('change', renderPlans);
    ...
    if (els.form) els.form.addEventListener('submit', saveChange);
    if (els.archiveBtn) els.archiveBtn.addEventListener('click', archiveCurrentChange);
  ...
  async function loadData(manual) {
    if (!SCRIPT_URL || SCRIPT_URL.includes('PASTE_YOUR_WORKING')) {
      setStatus('error', 'Missing Apps Script URL. Open config.js and paste your working /exec URL.');
      return;
    }
    ...
    try {
      ...
      if (!data || data.ok === false || data.success === false) {
        throw new Error(data && data.error ? data.error : 'Apps Script returned an error.');
      }
      ...
      state.plans = rawPlans
        .map(normalizePlan)
        .filter(p => p.name)
        .sort((a, b) => a.order - b.order || a.name.localeCompare(b.name));

      state.changes = rawChanges
        .map(normalizeChange)
        .filter(c => c.floorplan && isActive(c));

      state.typeOptions = [...new Set(state.changes.map(c => c.changeType).filter(Boolean))].sort();

      if (!state.selected && state.plans.length) {
        state.selected = state.plans[0].name;
      }

      populateTypeFilter();
      renderAll();

      setStatus('success', `Live data loaded from Google Sheet. ${state.plans.length} plans found.`);
    } catch (err) {
      console.error(err);
  ...
  function populateTypeFilter() {
    ...
    els.typeFilter.innerHTML =
      ...
      state.typeOptions.map(t => `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`).join('');
  ...
  function renderStats() {
    if (els.statPlans) els.statPlans.textContent = state.plans.length;
    if (els.statChanges) els.statChanges.textContent = activeChanges().length;
    if (els.statOpen) els.statOpen.textContent = activeChanges().filter(c => !isCompleteStatus(c.status)).length;
    if (els.statComplete) els.statComplete.textContent = activeChanges().filter(c => isCompleteStatus(c.status)).length;
  }
  ...
  function renderPlans() {
    ...
    els.planList.innerHTML = list.map(plan => {
      const changes = changesFor(plan.name);
      return `
        <button class="plan-card ${state.selected === plan.name ? 'active' : ''}" data-plan="${escapeHtml(plan.name)}" type="button">
          <div>
            <div class="plan-title">${escapeHtml(plan.name)}</div>
            <div class="plan-sub">${changes.length} change${changes.length === 1 ? '' : 's'} • Total sales: ${escapeHtml(plan.totalSales || '0')}</div>
          </div>
        </button>
      `;
    }).join('');

    $$('.plan-card', els.planList).forEach(btn => {
      btn.addEventListener('click', () => {
        state.selected = btn.dataset.plan;
        renderAll();
      });
    });
  }
  ...
  function renderDetails() {
    ...
    if (!plan) {
      els.details.innerHTML = `
        <div class="empty-main">
          ...
          <p>Select a floorplan on the left to view its change history.</p>
        </div>
      `;
      return;
    }

    const changes = changesFor(plan.name).sort((a, b) => {
      return String(b.changeDate).localeCompare(String(a.changeDate)) ||
    ...
    els.details.innerHTML = `
      <div class="plan-detail-head">
        <div>
          <div class="eyebrow">FLOORPLAN</div>
          <h2 class="plan-detail-title">${escapeHtml(plan.name)}</h2>
          <p>${escapeHtml(changes.length ? `${changes.length} logged change${changes.length === 1 ? '' : 's'}` : 'No changes logged yet')}</p>
        </div>
        <div class="detail-actions">
          <button id="addChangeBtn" class="btn primary" type="button">+ Add Change</button>
        </div>
      </div>

      <div class="detail-grid">
        <div class="info-card">
          <h3>Sales Agent Notes</h3>
          <p>${escapeHtml(plan.agentNotes || 'No sales agent notes listed.')}</p>
        </div>

        <div class="info-card">
          <h3>Sales History</h3>
          <div class="sales-row">
            ${salesPill('2025', plan.sales2025)}
            ${salesPill('2024', plan.sales2024)}
            ${salesPill('2023', plan.sales2023)}
            ${salesPill('Total', plan.totalSales)}
          </div>
        </div>
      </div>
      ...
      <div class="info-card">
        ...
        ${
          changes.length
            ? `<div class="change-list">${changes.map(changeCard).join('')}</div>`
            : '<div class="empty-state"><strong>No changes logged</strong><span>Click Add Change to create the first record for this floorplan.</span></div>'
        }
      </div>
    `;

    const addBtn = $('#addChangeBtn');
    if (addBtn) addBtn.addEventListener('click', () => openModal(plan.name));

    $$('.edit-change', els.details).forEach(btn => {
      btn.addEventListener('click', () => {
  ...
  function salesPill(label, value) {
    return `<div class="sales-pill"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value || '0')}</strong></div>`;
  }
  ...
  function changeCard(c) {
    ...
    return `
      <article class="change-card ${klass}">
        <div class="change-top">
          <div>
            <div class="change-title">${escapeHtml(c.description || '(No description)')}</div>
            <div class="meta">
              <span>${escapeHtml(c.changeDate || 'No date')}</span>
              <span>${escapeHtml(c.changeType || 'Other')}</span>
              ${c.costImpact ? `<span>${escapeHtml(c.costImpact)}</span>` : ''}
            </div>
          </div>
        ...
        <div class="note-block">
          <span class="status-chip ${klass}">${escapeHtml(status)}</span>
          ${c.notes ? `<p><strong>Notes:</strong> ${escapeHtml(c.notes)}</p>` : '<p><strong>Notes:</strong> No notes added yet.</p>'}
        </div>

        ${c.reason ? `<p><strong>Reason:</strong> ${escapeHtml(c.reason)}</p>` : ''}

        <div class="meta">
          <span>Requested: ${escapeHtml(c.requestedBy || 'N/A')}</span>
          <span>Approved: ${escapeHtml(c.approvedBy || 'N/A')}</span>
          <span>Plan Set: ${escapeHtml(c.planSetUpdated || 'No')}</span>
          <span>Pricing: ${escapeHtml(c.pricingUpdated || 'No')}</span>
          <span>Option: ${escapeHtml(c.optionUpdated || 'No')}</span>
        </div>
      </article>
    `;
  }
  ...
  function openModal(floorplan, change) {
    ...
    if (formTitle) formTitle.textContent = change ? `Edit ${floorplan} Change` : `Add ${floorplan} Change`;

    setValue('#changeId', change ? change.id : '');
    setValue('#formFloorplan', floorplan);
    setValue('#changeDate', change ? toInputDate(change.changeDate) : new Date().toISOString().slice(0, 10));
    setValue('#changeType', change ? (change.changeType || 'Design') : 'Design');
    setValue('#status', mapStatusForSelect(change ? change.status : 'Purchasing'));
    setValue('#costImpact', change ? (change.costImpact || '') : '');
    setValue('#description', change ? (change.description || '') : '');
    setValue('#reason', change ? (change.reason || '') : '');
    setValue('#requestedBy', change ? (change.requestedBy || '') : '');
    setValue('#approvedBy', change ? (change.approvedBy || '') : '');
    setValue('#planSetUpdated', change ? (change.planSetUpdated || 'No') : 'No');
    setValue('#pricingUpdated', change ? (change.pricingUpdated || 'No') : 'No');
    setValue('#optionUpdated', change ? (change.optionUpdated || 'No') : 'No');
    setValue('#notes', change ? (change.notes || '') : '');

    if (els.archiveBtn) els.archiveBtn.hidden = !change;
  ...
  function saveChange(event) {
    ...
    if (!SCRIPT_URL || SCRIPT_URL.includes('PASTE_YOUR_WORKING')) {
      alert('Paste your Apps Script URL in config.js first.');
      return;
    }
  ...
  function activeChanges() {
    return state.changes.filter(isActive);
  }

  function changesFor(name) {
    const n = String(name).toLowerCase();
    return activeChanges().filter(c => c.floorplan.toLowerCase() === n);
  }
  ...
  function isCompleteStatus(s) {
    return /^complete$/i.test(String(s || '').trim());
  }
  ...
  function upsertLocal(c) {
    const i = state.changes.findIndex(x => x.id === c.id);
    if (i >= 0) state.changes[i] = c;
    else state.changes.push(c);
  }

  function localChangeId(plan) {
    return 'CHG-' +
      new Date().toISOString().slice(0, 10).replace(/-/g, '') +
      '-' +
      slug(plan).slice(0, 3).toUpperCase() +
      '-' +
